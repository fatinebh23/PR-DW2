...
public class Product implements Comparable<Product> {
    ...
    public void writeFileEj6(String nomfich) {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(nomfich, "rws")) {
            ...
            if (!encontrado) {
                raf.seek(raf.length()); //nos posicionamos al final del fichero
                raf.writeBytes(producto); //escribirmos el producto
                raf.writeBytes("\n");//añadimos un salto de línea para que el próximo producto se escriba en la siguiente línea
            }
